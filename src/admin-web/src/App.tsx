...
const initialTargets: Target[] = [
  { id: 't-1001', type: 'group', name: '客户服务群 A', remark: 'VIP-A-20260910', account: '运营微信 01', service: '高级通用群服务', expiresAt: '2026-09-10', status: 'expiring' },
  { id: 't-1002', type: 'contact', name: '张先生', remark: 'C1024-正式-永久', account: '运营微信 01', service: '基础服务', expiresAt: '永久', status: 'active' },
  { id: 't-1003', type: 'group', name: '售后协作群', remark: '售后-B-20270208', account: '客服微信 02', service: '基础服务', expiresAt: '2027-02-08', status: 'active' },
  { id: 't-1004', type: 'contact', name: '李女士', remark: '待激活', account: '客服微信 02', service: '未激活', expiresAt: '-', status: 'inactive' },
  { id: 't-1005', type: 'group', name: '活动通知群', remark: '活动-C-20260818', account: '运营微信 01', service: '基础服务', expiresAt: '2026-08-18', status: 'expiring' },
]

const initialMentions: Mention[] = [
  { id: 'm-3021', group: '客户服务群 A', sender: '成员 018', content: '@机器人 查询服务到期时间', receivedAt: '19:41:08', priority: 'high', status: 'pending', service: '高级通用群服务' },
  { id: 'm-3020', group: '售后协作群', sender: '成员 007', content: '@机器人 转人工', receivedAt: '19:38:26', priority: 'high', status: 'pending', service: '基础服务' },
  { id: 'm-3019', group: '活动通知群', sender: '成员 042', content: '@机器人 帮助', receivedAt: '19:35:17', priority: 'normal', status: 'handled', service: '基础服务' },
  { id: 'm-3018', group: '未授权测试群', sender: '成员 003', content: '@机器人 激活一年', receivedAt: '19:31:52', priority: 'normal', status: 'blocked', service: '未激活' },
]

const initialEntitlements: Entitlement[] = [
  { id: 'E-202608-0318', target: '客户服务群 A', type: '群聊', plan: '高级通用群服务', period: '90 天', startsAt: '2026-06-12', endsAt: '2026-09-10', status: 'expiring' },
  { id: 'E-202608-0291', target: '张先生', type: '联系人', plan: '基础服务', period: '永久', startsAt: '2026-05-18', endsAt: '永久', status: 'active' },
  { id: 'E-202608-0254', target: '售后协作群', type: '群聊', plan: '基础服务', period: '半年', startsAt: '2026-08-08', endsAt: '2027-02-08', status: 'active' },
  { id: 'E-202608-0187', target: '内部演示群', type: '群聊', plan: '高级通用群服务', period: '30 天', startsAt: '2026-07-12', endsAt: '2026-08-11', status: 'expired' },
]

const initialBackups: Backup[] = [
  { id: 'BKP-0811-1800', createdAt: '2026-08-11 18:00', type: 'incremental', size: '12.4 MB', checksum: 'a81c...82f0', status: 'verified' },
  { id: 'BKP-0811-0200', createdAt: '2026-08-11 02:00', type: 'full', size: '184.7 MB', checksum: '91bd...1ca2', status: 'verified' },
  { id: 'BKP-0810-1800', createdAt: '2026-08-10 18:00', type: 'incremental', size: '11.8 MB', checksum: 'c021...713e', status: 'verified' },
]

const initialAudits: AuditRow[] = [
  ['AUD-88421', '19:47:12', '运营主管', '服务激活', '客户服务群 A', '成功'],
  ['AUD-88420', '19:41:09', '系统', '@ 权益判定', 'm-3021 · 允许', '成功'],
  ['AUD-88419', '19:36:44', '备注任务', '微信同步', '售后协作群', '跳过'],
  ['AUD-88418', '19:31:53', '系统', '激活请求拦截', '未授权测试群', '成功'],
  ['AUD-88417', '18:00:05', '备份服务', '增量备份', 'BKP-0811-1800', '成功'],
]

const initialSystemState: ApiSystemState = {
  tenantId: 'demo',
  name: '本地演示',
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="modal" role="dialog" aria-modal="true" aria-label={title}>
        <header className="modal-header"><h2>{title}</h2><button className="icon-button" type="button" title="关闭" onClick={onClose}><X size={18} /></button></header>
        {children}
      </section>
    </div>
  )
...
function DashboardPage({
  ...
  const changeAutomationState = async () => {
    if (!automationIntent || automationConfirmation !== expectedConfirmation || automationReason.trim().length < 3) return
  ...
  return (
    <div className="page-stack">
      <section className={`automation-band ${systemState.automationPaused ? 'paused' : ''}`}>
        <span className="automation-icon">{systemState.automationPaused ? <CirclePause size={20} /> : <ShieldCheck size={20} />}</span>
        <div><strong>{systemState.automationPaused ? '自动化已暂停' : '自动化运行许可已开启'}</strong><span>{apiConnected ? `状态版本 ${systemState.version} · ${formatDate(systemState.updatedAt)} ${formatTime(systemState.updatedAt)}` : '本地演示状态'}</span></div>
        <button className={systemState.automationPaused ? 'primary-button' : 'danger-button'} disabled={!apiConnected} onClick={() => { setAutomationIntent(systemState.automationPaused ? 'resume' : 'pause'); setAutomationError('') }}>{systemState.automationPaused ? <Play size={16} /> : <CirclePause size={16} />}{systemState.automationPaused ? '申请恢复' : '紧急暂停'}</button>
      </section>
      ...
      <div className="dashboard-grid">
        ...
        <section className="panel alert-panel">
          ...
          <div className="alert-list">
            {!apiConnected && <div className="alert-row critical"><CircleAlert size={18} /><div><strong>后端未连接</strong><span>当前为本地演示数据</span></div><time>现在</time></div>}
      ...
      <section className="panel">
        ...
        <div className="table-wrap"><table><thead><tr><th>队列</th><th>总数</th><th>待处理</th><th>异常/冲突</th><th>状态</th></tr></thead><tbody>
          <tr><td className="strong-cell">群 @ 事件</td><td>{mentions.length}</td><td>{pendingMentions.length}</td><td>{mentions.filter((item) => item.status === 'blocked').length}</td><td><StatusBadge status={pendingMentions.length ? 'pending' : 'active'} /></td></tr>
          <tr><td className="strong-cell">自动备注任务</td><td>{remarkTasks.length}</td><td>{pendingRemarks.length}</td><td>{failedRemarks.length}</td><td><StatusBadge status={failedRemarks.length ? 'warning' : pendingRemarks.length ? 'pending' : 'active'} /></td></tr>
          <tr><td className="strong-cell">联系人与群</td><td>{targets.length}</td><td>{targets.filter((item) => item.status === 'inactive').length}</td><td>{targets.filter((item) => item.status === 'expiring').length}</td><td><StatusBadge status={targets.some((item) => item.status === 'expiring') ? 'warning' : 'active'} /></td></tr>
        </tbody></table></div>
      </section>
      {automationIntent && <Modal title={automationIntent === 'pause' ? '暂停全部自动化' : '恢复自动化许可'} onClose={() => setAutomationIntent(null)}><div className="modal-body"><div className="danger-notice"><CircleAlert size={18} /><div><strong>{automationIntent === 'pause' ? '所有 Agent 将收到急停状态' : '恢复许可不会绕过 Agent 本地安全门禁'}</strong><span>{automationIntent === 'pause' ? '新的自动备注与已激活群 @ 处理将被控制面拦截。' : '未知微信版本、窗口身份不确定或只读构建仍会保持暂停。'}</span></div></div><label className="field"><span>变更原因</span><input aria-label="变更原因" value={automationReason} onChange={(event) => setAutomationReason(event.target.value)} maxLength={500} autoFocus /></label><label className="field"><span>输入 {expectedConfirmation} 确认</span><input aria-label={`输入 ${expectedConfirmation} 确认`} value={automationConfirmation} onChange={(event) => setAutomationConfirmation(event.target.value)} autoComplete="off" /></label>{automationError && <div className="error-inline"><CircleAlert size={16} />{automationError}</div>}</div><footer className="modal-footer"><button className="secondary-button" onClick={() => setAutomationIntent(null)}>取消</button><button className={automationIntent === 'pause' ? 'danger-button' : 'primary-button'} disabled={automationBusy || automationReason.trim().length < 3 || automationConfirmation !== expectedConfirmation} onClick={() => void changeAutomationState()}>{automationBusy ? '处理中' : automationIntent === 'pause' ? '确认暂停' : '确认恢复'}</button></footer></Modal>}
    </div>
  )
}
...
function TargetsPage({
  ...
  const openEditor = (target: Target) => {
    ...
    setEditing(target); setRemark(target.remark === '未设置' ? '' : target.remark); setRuleId(firstRule?.id ?? ''); setError('')
  }
  ...
  const saveRemark = async () => {
    ...
    try {
      if (apiConnected) {
        ...
        setTargets((items) => items.map((item) => item.id === editing.id ? { ...item, remark: remark.trim() } : item))
        setNotice('演示备注已更新')
      }
      setEditing(null)
    } catch (queueError) {
  ...
  return (
    <div className="page-stack">
      {notice && <div className="toast-inline"><Check size={16} />{notice}<button title="关闭" onClick={() => setNotice('')}><X size={15} /></button></div>}
      <section className="panel data-panel">
        <div className="toolbar">
          <div className="segmented"><button className={tab === 'group' ? 'active' : ''} onClick={() => setTab('group')}><Group size={16} />群聊</button><button className={tab === 'contact' ? 'active' : ''} onClick={() => setTab('contact')}><ContactRound size={16} />联系人</button></div>
          <div className="toolbar-actions"><label className="search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索名称、备注或账号" /></label></div>
        </div>
        <div className="table-wrap"><table><thead><tr><th>{tab === 'group' ? '群聊' : '联系人'}</th><th>系统备注</th><th>微信标识</th><th>服务</th><th>到期时间</th><th>备注任务</th><th>状态</th><th /></tr></thead><tbody>
          {filtered.map((target) => { const task = latestTask(target.id); const pending = task?.status === 'pending'; return <tr key={target.id}><td><div className="identity"><span className={`identity-icon ${target.type}`}>{target.type === 'group' ? <Group size={16} /> : <ContactRound size={16} />}</span><div><strong>{target.name}</strong><small>{target.id}</small></div></div></td><td className="mono-cell">{target.remark}</td><td>{target.account}</td><td>{target.service}</td><td>{target.expiresAt}</td><td>{task ? <div><StatusBadge status={task.status} /><small className="cell-subtitle">{task.generatedRemark}</small></div> : '暂无'}</td><td><StatusBadge status={target.status} /></td><td><button className="icon-button small" title={pending ? '已有待执行备注任务' : '创建备注同步任务'} disabled={pending} onClick={() => openEditor(target)}><Pencil size={15} /></button></td></tr> })}
        </tbody></table></div>
        <footer className="table-footer"><span>共 {filtered.length} 条</span><span>{apiConnected ? `后端任务 ${remarkTasks.length} 个` : '本地演示模式'}</span></footer>
      </section>
      {editing && <Modal title={apiConnected ? '创建备注同步任务' : '编辑演示备注'} onClose={() => setEditing(null)}><div className="modal-body"><div className="target-summary"><span className={`identity-icon ${editing.type}`}>{editing.type === 'group' ? <Group size={17} /> : <ContactRound size={17} />}</span><div><strong>{editing.name}</strong><small>{editing.account} · {editing.id}</small></div></div>{apiConnected ? <label className="field"><span>备注规则</span><select aria-label="备注规则" value={ruleId} onChange={(event) => setRuleId(event.target.value)} autoFocus><option value="">选择已启用的规则</option>{matchingRules.map((rule) => <option value={rule.id} key={rule.id}>{rule.name} · {rule.template}</option>)}</select></label> : <label className="field"><span>备注内容</span><input aria-label="备注内容" value={remark} onChange={(event) => setRemark(event.target.value)} maxLength={40} autoFocus /><small>{remark.length} / 40</small></label>}<div className="notice"><ShieldCheck size={17} /><span>{apiConnected ? '任务进入后端队列后，只有通过微信目标身份二次校验并由 Agent 回报一致结果，系统备注才会更新。' : '本地演示不会创建后端任务或修改微信。'}</span></div>{error && <div className="danger-notice"><CircleAlert size={18} /><div><strong>创建失败</strong><span>{error}</span></div></div>}</div><footer className="modal-footer"><button className="secondary-button" onClick={() => setEditing(null)}>取消</button><button className="primary-button" onClick={() => void saveRemark()} disabled={submitting || (apiConnected ? !ruleId : !remark.trim())}><Check size={16} />{submitting ? '处理中' : '确认排队'}</button></footer></Modal>}
    </div>
  )
}

function MentionsPage({ mentions, setMentions, apiConnected }: { mentions: Mention[]; setMentions: React.Dispatch<React.SetStateAction<Mention[]>>; apiConnected: boolean }) {
  const [status, setStatus] = useState<'all' | Mention['status']>('all')
  const items = mentions.filter((item) => status === 'all' || item.status === status)
  const handle = (id: string) => setMentions((current) => current.map((item) => item.id === id ? { ...item, status: 'handled' } : item))
  return <section className="panel data-panel"><div className="toolbar"><div className="segmented compact"><button className={status === 'all' ? 'active' : ''} onClick={() => setStatus('all')}>全部</button><button className={status === 'pending' ? 'active' : ''} onClick={() => setStatus('pending')}>待处理</button><button className={status === 'handled' ? 'active' : ''} onClick={() => setStatus('handled')}>已处理</button><button className={status === 'blocked' ? 'active' : ''} onClick={() => setStatus('blocked')}>已拦截</button></div></div><div className="mention-list">
    {items.map((item) => <article className="mention-row" key={item.id}><span className={`priority-marker ${item.priority}`} /><div className="mention-main"><div className="mention-meta"><strong>{item.group}</strong><span>{item.sender}</span><time>{item.receivedAt}</time></div><p>{item.content}</p><div className="mention-tags"><span>{item.service}</span><span>{item.id}</span></div></div><div className="mention-actions"><StatusBadge status={item.status} />{!apiConnected && item.status === 'pending' && <button className="primary-button small-button" onClick={() => handle(item.id)}><Check size={15} />演示标记</button>}</div></article>)}
...
function ServicesPage({
  ...
  entitlements: Entitlement[]
  setEntitlements: React.Dispatch<React.SetStateAction<Entitlement[]>>
  targets: Target[]
  apiConnected: boolean
  onActivated: (input: { targetId: string; targetKind: TargetType; packageCode: 'BASIC' | 'ADVANCED_GENERAL'; duration: ApiEntitlement['duration'] }) => Promise<void>
}) {
  ...
  const activate = async () => {
    ...
    const endDates: Record<string, string> = { '30 天': '2026-09-10', '60 天': '2026-10-10', '90 天': '2026-11-09', '半年': '2027-02-11', '一年': '2027-08-11', '永久': '永久' }
        setEntitlements((items) => [{ id: `E-${Date.now().toString().slice(-8)}`, target: selected.name, type: selected.type === 'group' ? '群聊' : '联系人', plan, period, startsAt: '2026-08-11', endsAt: endDates[period], status: 'active' }, ...items])
      }
  ...
  return <div className="page-stack"><section className="metrics service-metrics"><div className="metric"><span>有效权益</span><strong>{activeCount}</strong><small>基础与高级通用服务</small></div><div className="metric"><span>即将到期</span><strong>{expiringCount}</strong><small className="attention">30 天内</small></div><div className="metric"><span>永久授权</span><strong>{permanentCount}</strong><small>不含维护期限</small></div><div className="metric"><span>数据来源</span><strong>{apiConnected ? '实时' : '演示'}</strong><small className="positive">{apiConnected ? '权益流水已连接' : '后端连接后切换'}</small></div></section><section className="panel data-panel"><div className="toolbar"><strong>权益实例</strong><div className="toolbar-actions"><label className="search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索服务单或目标" /></label><button className="primary-button" onClick={() => setModal(true)}><Plus size={16} />开通服务</button></div></div><div className="table-wrap"><table><thead><tr><th>服务单</th><th>授权目标</th><th>套餐</th><th>期限</th><th>开始时间</th><th>结束时间</th><th>状态</th></tr></thead><tbody>{filteredEntitlements.map((item) => <tr key={item.id}><td className="mono-cell">{item.id}</td><td><strong>{item.target}</strong><small className="cell-subtitle">{item.type}</small></td><td>{item.plan}</td><td>{item.period}</td><td>{item.startsAt}</td><td>{item.endsAt}</td><td><StatusBadge status={item.status} /></td></tr>)}</tbody></table></div>{filteredEntitlements.length === 0 && <div className="empty-state">没有匹配的权益记录</div>}</section>
    {modal && <Modal title="开通服务" onClose={() => setModal(false)}><div className="modal-body form-grid"><label className="field full"><span>授权目标</span><select aria-label="授权目标" value={targetId} onChange={(event) => setTargetId(event.target.value)} autoFocus><option value="">选择已验证的群聊或联系人</option>{targets.map((item) => <option value={item.id} key={item.id}>{item.name} · {item.type === 'group' ? '群聊' : '联系人'}</option>)}</select></label><label className="field"><span>服务套餐</span><select aria-label="服务套餐" value={plan} onChange={(event) => setPlan(event.target.value)}><option>基础服务</option><option>高级通用群服务</option></select></label><label className="field"><span>服务期限</span><select aria-label="服务期限" value={period} onChange={(event) => setPeriod(event.target.value)}><option>30 天</option><option>60 天</option><option>90 天</option><option>半年</option><option>一年</option><option>永久</option></select></label><div className="activation-preview full"><KeyRound size={18} /><div><strong>{plan} · {period}</strong><span>生效前将校验目标绑定、激活码和现有权益。</span></div></div>{error && <div className="danger-notice full"><CircleAlert size={18} /><div><strong>开通失败</strong><span>{error}</span></div></div>}</div><footer className="modal-footer"><button className="secondary-button" onClick={() => setModal(false)}>取消</button><button className="primary-button" onClick={() => void activate()} disabled={!targetId || submitting}><PackageCheck size={16} />{submitting ? '处理中' : '确认开通'}</button></footer></Modal>}
  </div>
}
...
function BackupsPage({
  ...
  const createBackup = async () => {
    ...
    const item: Backup = { id: `BKP-${Date.now().toString().slice(-10)}`, createdAt: '2026-08-11 19:52', type: 'incremental', size: '计算中', checksum: '生成中', status: 'running' }
    setBackups((items) => [item, ...items]); setNotice('备份任务已进入队列')
    window.setTimeout(() => setBackups((items) => items.map((current) => current.id === item.id ? { ...current, size: '0.8 MB', checksum: 'f013...90ac', status: 'verified' } : current)), 900)
    setBusy(false)
  }
  const performRestore = async () => {
    ...
    setBusy(true); setError('')
    try {
      if (apiConnected) await onRestore(restore.id)
  ...
  return <div className="page-stack"><section className="backup-summary"><div><ShieldCheck size={21} /><span>备份策略</span><strong>正常</strong><small>每次恢复前自动再备份</small></div><div><DatabaseBackup size={21} /><span>最近备份</span><strong>{backups.length ? backups[0].createdAt : '暂无'}</strong><small>{apiConnected ? '后端清单已连接' : '本地演示数据'}</small></div><div><ArchiveRestore size={21} /><span>恢复模式</span><strong>受控合并</strong><small>修改当前库并强制暂停自动化</small></div></section>{notice && <div className="toast-inline"><Check size={16} />{notice}<button title="关闭" onClick={() => setNotice('')}><X size={15} /></button></div>}{error && <div className="error-inline"><CircleAlert size={16} />{error}<button title="关闭" onClick={() => setError('')}><X size={15} /></button></div>}<section className="panel data-panel"><div className="panel-header toolbar-heading"><div><h2>备份记录</h2><p>恢复会修改当前库；机器人保持暂停，旧发送任务不会自动重放</p></div><button className="primary-button" disabled={busy} onClick={() => void createBackup()}><DatabaseBackup size={16} />{busy ? '处理中' : '立即备份'}</button></div><div className="table-wrap"><table><thead><tr><th>备份编号</th><th>创建时间</th><th>类型</th><th>大小</th><th>校验和</th><th>状态</th><th /></tr></thead><tbody>{backups.map((backup) => <tr key={backup.id}><td className="mono-cell">{backup.id}</td><td>{backup.createdAt}</td><td>{backup.type === 'full' ? '全量' : '逻辑备份'}</td><td>{backup.size}</td><td className="mono-cell">{backup.checksum}</td><td><StatusBadge status={backup.status} /></td><td><button className="secondary-button small-button" disabled={backup.status !== 'verified'} onClick={() => setRestore(backup)}><ArchiveRestore size={15} />恢复</button></td></tr>)}</tbody></table></div></section>
    {restore && <Modal title="执行受控合并恢复" onClose={() => { setRestore(null); setConfirm('') }}><div className="modal-body"><div className="danger-notice"><CircleAlert size={19} /><div><strong>此操作会直接修改当前数据库</strong><span>系统先自动创建恢复前备份，再覆盖联系人、群和备注规则配置；权益、兑换和流水仅补缺，不会用旧快照覆盖现有事实。恢复后自动化保持暂停。</span></div></div><dl className="restore-details"><div><dt>备份编号</dt><dd>{restore.id}</dd></div><div><dt>创建时间</dt><dd>{restore.createdAt}</dd></div><div><dt>完整性</dt><dd>校验通过</dd></div></dl><label className="field"><span>输入 RESTORE 确认</span><input aria-label="输入 RESTORE 确认" value={confirm} onChange={(event) => setConfirm(event.target.value)} autoComplete="off" /></label></div><footer className="modal-footer"><button className="secondary-button" onClick={() => setRestore(null)}>取消</button><button className="danger-button" disabled={confirm !== 'RESTORE' || busy} onClick={() => void performRestore()}><ArchiveRestore size={16} />{busy ? '处理中' : '确认恢复当前库'}</button></footer></Modal>}
  </div>
}
...
function App() {
  ...
  const [systemState, setSystemState] = useState<ApiSystemState>(initialSystemState)
  ...
  const refreshData = useCallback(async () => {
    setRefreshing(true)
    try {
      const mapped = mapSnapshot(await loadConsoleSnapshot())
  ...
  const handleActivation = async (input: { targetId: string; targetKind: TargetType; packageCode: 'BASIC' | 'ADVANCED_GENERAL'; duration: ApiEntitlement['duration'] }) => {
    await activateTarget(input)
    await refreshData()
  }
  const handleCreateBackup = async () => { await createLogicalBackup('管理台手工备份'); await refreshData() }
  const handleRestore = async (id: string) => { await createControlledMergeRestore(id); await refreshData() }
  const handleQueueRemark = async (ruleId: string, targetId: string) => {
    const task = await queueRemarkTask(ruleId, targetId)
    await refreshData()
    return task
  }
  const handleAutomationChange = async (paused: boolean, reason: string) => {
    const updated = await setAutomationState(systemState, paused, reason)
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? 'open' : ''}`}>
        ...
        <nav>{navItems.map((item) => { const Icon = item.icon; return <button key={item.id} className={page === item.id ? 'active' : ''} onClick={() => navigate(item.id)}><Icon size={18} /><span>{item.label}</span>{item.id === 'mentions' && pendingMentions > 0 && <b>{pendingMentions}</b>}</button> })}</nav>
        <div className="sidebar-footer"><div className="operator"><span>管</span><div><strong>平台管理员</strong><small>{apiConnected ? '开发环境 · API 已连接' : '本地演示'}</small></div><ChevronDown size={15} /></div></div>
      </aside>
      {mobileNav && <button className="nav-scrim" aria-label="关闭导航" onClick={() => setMobileNav(false)} />}
      <main className="main-area">
        <header className="topbar"><div className="title-row"><button className="icon-button menu-button" title="打开导航" onClick={() => setMobileNav(true)}><Menu size={20} /></button><div><h1>{pageTitles[page].title}</h1><p>{pageTitles[page].subtitle}</p></div></div><div className="topbar-actions"><span className={`environment ${apiConnected ? '' : 'offline'}`}><Activity size={14} />{apiConnected ? 'API 已连接' : '本地演示'}</span><button className={`icon-button bordered ${refreshing ? 'spinning' : ''}`} title="刷新数据" onClick={() => void refreshData()}><RefreshCw size={17} /></button></div></header>
        <div className="page-content">
          {apiError && <div className="connection-banner"><CircleAlert size={16} /><span>后端未连接，当前显示本地演示数据：{apiError}</span><button onClick={() => void refreshData()}>重试</button></div>}
          ...
          {page === 'targets' && <TargetsPage targets={targets} setTargets={setTargets} remarkRules={remarkRules} remarkTasks={remarkTasks} apiConnected={apiConnected} onQueueRemark={handleQueueRemark} />}
          {page === 'mentions' && <MentionsPage mentions={mentions} setMentions={setMentions} apiConnected={apiConnected} />}
          {page === 'services' && <ServicesPage entitlements={entitlements} setEntitlements={setEntitlements} targets={targets} apiConnected={apiConnected} onActivated={handleActivation} />}
          {page === 'backups' && <BackupsPage backups={backups} setBackups={setBackups} apiConnected={apiConnected} onCreate={handleCreateBackup} onRestore={handleRestore} />}
